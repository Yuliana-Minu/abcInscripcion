<?php
    ...
    class Csso extends Acceso{
        ...
        public function acceso(){ //$user, $paswd
            ...
            /*
            $sql_actualizacion="SELECT eni_codigo, eni_ninio, eni_estado,
                                       eni_estadoproceso
                                 FROM principal.estado_ninio
                               WHERE eni_ninio=".$datos['per_codigo'].";";

            $consulta_actualizacion=$cnxion->ejecutar($sql_actualizacion);
            $datos_actualizacion=$cnxion->obtener_filas($consulta_actualizacion);

            $this->actualizacion=$datos_actualizacion['eni_estado'];
            $this->estadoProceso=$datos_actualizacion['eni_estadoproceso'];
            
            if($numero > 0){
                $this->entroAcceso = "acceso_ok-".$datos_actualizacion['eni_estado']."-".$datos_actualizacion['eni_estadoproceso'];
            }
            else{
                $this->entroAcceso = "acceso_nok".$pass;
            }
